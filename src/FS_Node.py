...
mensagemFiles = f"files . {' | '.join(nomesFicheiros)}" 

# Envia a mensagem para o servidor
node_socket.send(mensagemFiles.encode())
...
while True:
    ...
    if comando[0] == "quit":
        ...
        node_socket.send(mensagemGet.encode())
        resposta = node_socket.recv(1024).decode()
        print(resposta)
        print(nomesFicheiros)
